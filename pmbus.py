...
class PMBus:
    ...
    def _writeWordPMBus(self, cmd, word, pecByte=True):
        ...
        if (self.USE_PyMCP2221A == True):
            for attempt in range(10):
                try:
                    self.bus.write_word_data(self.address, cmd, word)
                    break
    ...
    def _readBytePMBus(self, cmd, pecByte=True):
        ...
        if (self.USE_PyMCP2221A == True):
            for attempt in range(10):
                try:
                    data = self.bus.read_byte_data(self.address, cmd)
                    return data
                except Exception as e:
                    continue
        else:
    ...
    def _readBlockPMBus(self, cmd, pecByte=True, force=None):
        if (self.USE_PyMCP2221A == True):
            for attempt in range(10):
                try:
                    data = self.bus.read_i2c_block_data(self.address, cmd,32)
                    return data
                except Exception as e:
                    continue
        else:
    ...
    def setVoutTrim(self,val):
        value_to_write = val * (2 ** -self.VOUT_N)
        self._writeWordPMBus(0x22, value_to_write)
